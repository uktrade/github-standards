...
class TestPresidioScanResult:
    ...
    def test_str_output_for_empty_scan_result(self):
        result = PresidioScanResult()
        assert str(result) == "--------PERSONAL DATA SCAN SUMMARY--------"

    def test_str_output_for_error(self):
        result = PresidioScanResult()

        result.add_path_scan_result(
            PathScanResult(
                "a.txt",
                PathScanStatus.ERRORED,
                additional_detail="The reason for an exception being raised to help debugging",
            )
        )
        errored_paths_table = PrettyTable(["Path", "Reason"])
        errored_paths_table.add_row(["a.txt", "The reason for an exception being raised to help debugging"])

        assert str(errored_paths_table) in str(result)
