...
build-docker:
	docker build . -t github-standards-hooks:dev

build-docker-local-testing:
	docker build . -t github-standards-hooks:local-testing --target local-testing --file local.Dockerfile
...
validate-hook-docker:
	...
	docker run --rm github-standards-hooks:local-testing --hook-id validate-security-scan --verbose EXAMPLE_COMMIT_MSG.txt
...
run-hook-docker:
	make build-docker-local-testing
	docker run --rm github-standards-hooks:local-testing --hook-id run-security-scan --verbose src/hooks_base.py